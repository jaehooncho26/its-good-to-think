...
(() => {
  ...
  // --- SAME NOTIFICATION STYLE (single OK button) ---
  function showPopupOk(messageHTML, onOk) {
    ...
    overlay.innerHTML = `
      <div class="tier-alert-box">
        ...
        <div class="tier-alert-buttons">
          <img src="ok-button.png" id="okOnlyBtn" class="tier-alert-img" alt="OK">
        </div>
      </div>
    `;
    ...
    document.getElementById('okOnlyBtn').addEventListener('click', () => {
  ...
  async function onSpinClicked(username) {
    ...
    try {
      ...
      if (coins < 1) {
        showPopupOk('You need at least 1 Thought Coin to spin.');
        return;
      }
      ...
      if (!res.ok || !res.row) {
        ...
        if (msg.includes('NO_COINS')) {
          showPopupOk('You have 0 Thought Coins. Earn more coins to spin!');
        } else {
          showPopupOk('Spin failed. Please try again.');
        }
        return;
      }

      // Update dynamic-state
      if (rewards[idx] === 500) hotCooldown = HOT_COOLDOWN_SPINS;
      else if (hotCooldown > 0) hotCooldown -= 1;
      ...
      showPopupOk('Oops—something went wrong. Please try again.');
    } finally {
      isSpinning = false;
      setSpinEnabled(true);
    }
